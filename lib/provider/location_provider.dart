...
class LocationProvider with ChangeNotifier {
  ...
  void deleteUserAddressByID(String id, int index, Function callback) async {
    ResponseModel _responseModel;
    FireBaseResponse fireBaseResponse = await locationRepo.removeAddressByID(id);
    if(fireBaseResponse.responseCode == 200){
      _addressList.removeAt(index);
      callback(true, 'Deleted address successfully');
    }else{
      callback(false, fireBaseResponse.error);
    }
    //---------------
    /*
  ...
  Future<ResponseModel> initAddressList(BuildContext context) async {
    ResponseModel _responseModel;
    FireBaseResponse fireBaseResponse = await locationRepo.getAllAddress();
    if(fireBaseResponse.responseCode == 200){
      _addressList = fireBaseResponse.response;
      print('aaaaaaaaaaaaaaaa');
      print(_addressList[0].id);
      print(_addressList[0].contactPersonName);
  ...
  Future<ResponseModel> addAddress(AddressModel addressModel) async {
    ...
    ResponseModel _responseModel;
    FireBaseResponse fireBaseResponse = await locationRepo.addAddress(addressModel);
    print('++++++++++++++++++++++++++++++++');
    print(fireBaseResponse.responseCode);
    if(fireBaseResponse.responseCode == 200){
  ...
  Future<ResponseModel> updateAddress(BuildContext context, {AddressModel addressModel, String addressId}) async {
    _isLoading = true;
    notifyListeners();
    _errorMessage = '';
    _addressStatusMessage = null;
    ...
    FireBaseResponse fireBaseResponse = await locationRepo.updateAddress(addressModel, addressId);
    if(fireBaseResponse.responseCode == 200){
      initAddressList(context);
      responseModel = ResponseModel(true, fireBaseResponse.response);
    }else{
      responseModel = ResponseModel(false, fireBaseResponse.error);
    }
    ///-----------------------------------
    /*
